...
function Login() {
  ...
  const handleOpen = () => {
    appDispatch({ type: "showMenu", data: "login" });
  };
  ...
  return (
    <div>
      ...
      <Modal open={appState.visibleMenu === "login"} onClose={handleClose}>
        <div className="flex justify-center items-center h-screen">
          <div className="bg-white p-6 sm:p-12 shadow shadow-white rounded-xl w-[300px] sm:w-[450px]">
            ...
            <p onClick={() => appDispatch({type: "showMenu", data: "password"})} className="text-right text-[#a3aab5] hover:text-black duration-300 cursor-pointer mb-3">Forgot your password?</p>
            <button onClick={handleLogin} className="text-white mb-2 bg-brownOrange block w-full hover:bg-secondary duration-300 sm:text-lg font-semibold py-1 px-8 rounded-3xl">Sign In</button>
            ...
            <div className="flex justify-center gap-2">
              ...
              <button onClick={() => appDispatch({type: "showMenu", data: "signup"})} className="text-brownOrange hover:text-secondary duration-300  font-medium">Sign Up</button>
            </div>
          </div>
        </div>
      </Modal>
